...
@service_route.post("/")
async def add_new_service(request: Request,req: ServiceAddRequest, db=Depends(get_db),user=Depends(manager), csrf_protect:CsrfProtect = Depends()):
...
async def update_service(request: Request,service_id:int,req: ServiceUpdateRequest, db=Depends(get_db),user=Depends(manager), csrf_protect:CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token)
    new_service = Service(**{"SERVICE_NM": req.name, "SERVICE_DOMAIN": req.domain, "SERVICE_INDEX": req.index})
...
async def delete_service(request: Request,service_id:int,db=Depends(get_db),user=Depends(manager), csrf_protect:CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token)
